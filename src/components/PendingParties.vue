  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>Pending Parties</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
        ...
        <v-dialog v-model="dialog" max-width="500px">
          ...
          <v-card>
            <v-card-title>
              <span class="headline">Pending Parties</span>
            </v-card-title>
    ...
    <template v-slot:item.action="{ item }">
      <v-btn text @click="allow">Allow</v-btn>
      <v-btn text @click="allow">Disallow</v-btn>
      <v-icon
        small
        class="mr-2"
        @click="editItem(item)"
